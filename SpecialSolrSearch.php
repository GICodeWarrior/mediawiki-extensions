...
//        $search = str_replace("\n", " ", $wgRequest->getText('solrsearch', $titleParam));
    ...
    public function showFieldSets() {
        global $wgOut, $wgUser, $wgDisableTextSearch, $wgContLang, $wgScript, $wgSolrFields;
    ...
    public function showResults( $fieldSet ) {
        ...
        if ( trim( $query ) === '' || $filePrefix === trim( $query ) ) {
            $wgOut->addHTML( $this->formHeader( $query, 0, 0 ) );
            $wgOut->addHTML( '</form>' );
    ...
    protected function formHeader( $fieldSet, $resultsShown, $totalNum ) {
        global $wgLang;
