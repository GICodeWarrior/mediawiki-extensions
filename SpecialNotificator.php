...
function execute($par) {
	...
	if(! $pageId || ! $revId || ! $receiver ) {
		...
		if(! Sanitizer::validateEmail($receiver)) {
			$output = '<span class="error">' . wfMsg('e-mail-address-invalid') . ' ' . wfMsg('notification-not-sent') . '</span>';
		...
		if($oldRevId >= 0) {
			if($oldRevId > 0) {
				...
				$notificationText = wfMsg('notification-text-changes', htmlspecialchars($wgUser->getName()), '<a href="' . $linkToPage . '">' . $pageTitle . '</a>') . '<div style="margin-top: 1em;">' . $diff . '</div>';
			} else {
				// Receiver has never been notified about this page - so don't send a diff, just the link
				$mailSubjectPrefix = '[' . wfMsg('new-tag') . '] ';
				$notificationText = wfMsg('notification-text-new', htmlspecialchars($wgUser->getName()), '<a href="' . $linkToPage . '">' . $pageTitle . '</a>');
			}
			...
			if (Notificator::sendNotificationMail($receiver, $mailSubject, $notificationText)) {
				$output = '<strong>' . wfMsg('following-e-mail-sent-to', htmlspecialchars($receiver)) . '</strong><div style="margin-top: 1em;"><h3>' . wfMsg('subject') . ' ' . $mailSubject . '</h3><p>' . $notificationText . '</p></div>';
				Notificator::recordNotificationInDatabase($pageId, $revId, $receiver);
			} else {
				$output = '<span class="error">' . wfMsg('error-sending-e-mail', htmlspecialchars($receiver)) . '</span>';
			}
		} elseif($oldRevId == -1) {
			$output = '<span class="error">' . wfMsg('error-parameter-missing') . '</span>';
		} elseif($oldRevId == -2) {
			$output = '<strong>' . wfMsg('notified-already', htmlspecialchars($receiver)) . ' ' . wfMsg('notification-not-sent') . '</strong>';
		}
