#!/usr/local/bin/perl
# -i "w:/# Out Bayes" -o "w:/@ Report Card/Data"
  ...
  $month_0 = "08" ;   # collect 13 months up to
  $year_0  = 2010 ;
...
# $file_regions_Reach = "Multi-Country Media Trend, % reach by region (July 2008 - September 2009)_10786.csv" ;
  ...
  $maxpopularwikis = 25 ;
  @projects  = ('wb','wk','wn','wp','wq','ws','wv','commons') ;
  @projects2 = ('wb','wk','wn','wp','wq','ws','wv','wx','tot') ;
  @projects2b = ('Wikibooks','Wiktionary','Wikinews','Wikipedia','Wikiquote','Wikisource','Wikiversity','Other','Total') ;
...
#  die "Output folder '$path_out' does not exist" if (! -d $path_out) ;
  ...
  $file_csv_out = "$path_out/StatisticsMonthly_${year_0}_$month_0.csv" ;
  $file_csv_out_year = "$path_out/StatisticsYearly.csv" ;

  &SetComparisonPeriods ($year_0,$month_0) ;
}
...
if ($month == 6)
{
  @fields = split (',', $counts) ;
  $articles = $fields [4] ;
  $june_articles {"$project,$year"} += $articles ;
  $june_articles {"tot,$year"} += $articles ;
  $years {$year} ++ ;
# print "$project $year $month : $articles\n" ;
}

    my $m = &MonthsSinceYearAgo ($year, $month) ;
    if (! ($m < 0) || ($m > 12))
    {
  ...
  foreach $language (sort keys %languages)
  {
    ...
    for ($m=0 ; $m <= 12 ; $m++)
    {
      ...
      foreach $f (2,3) # editors_gt_5, editors_gt_100
      {
        ...
        if ($language ne 'commons')              # ignore editor count on commons for totals, most editors are already counted for other project
        { $totals {"$f,$m"} += $fields [$f2] ; } # (even for several projects, to be tuned after centralauth dump is available)
...
sub WriteYearlyData
{
  print "Write file '$file_csv_out_year'\n" ;
  open CSV_OUT, '>', $file_csv_out_year ;

  $line = "Articles per project per year (June 30)" ;
  print         "$line\n" ;
  print CSV_OUT "$line\n" ;

  $line = "" ;
  foreach $project (@projects2b)
  { $line .= ",$project," ; }
  $line .= ",Growth," ;
  print         "$line\n" ;
  print CSV_OUT "$line\n" ;
  ...
  {
    $june_diff {$year} = $june_articles {"tot,$year"} - $june_articles {"tot,".($year-1)} ;
    $line = "$year" ;
    foreach $project (@projects2)
    {
      $count      = $june_articles {"$project,$year"} ;
      $count_prev = $june_articles {"$project,".($year-1)} ;
      $perc = "-" ;
      if ($count_prev > 0)
      { $perc = sprintf ("%.0f", 100 * ($count/$count_prev) - 100) . '%' ;  $perc =~ s/^(\d)/\+$1/ ; }
    # $count = sprintf ("%.0f", $count / 1000) ;
      $line .= ",$count,$perc" ;
    }
    $diff      = $june_diff {$year} ;
    $diff_prev = $june_diff {$year-1} ;
    $perc = "-" ;
    if ($diff_prev > 0)
    { $perc = sprintf ("%.0f", 100 * ($diff/$diff_prev) - 100) . '%' ; $perc =~ s/^(\d)/+$1/ ;  }
    $line .= ",$diff,$perc" ;
    print         "$line\n" ;
    print CSV_OUT "$line\n" ;
  }
  ...
  foreach $year (sort keys %years)
  {
    ...
    foreach $project (@projects2)
    {
      ...
      { $perc = sprintf ("%.0f", 100 * ($count/$count_prev) - 100) . '%' ;  $perc =~ s/^(\d)/\+$1/ ; }
      if ($count >= 1000000)
      { $count = sprintf ("%.1f", $count / 1000000) . 'M'  ; }
      elsif ($count >= 1000)
      { $count = sprintf ("%.0f", $count / 1000) . 'k'  ; }
      $line .= ",$count,$perc" ;
    }
    $diff      = $june_diff {$year} ;
    $diff_prev = $june_diff {$year-1} ;
    ...
    { $perc = sprintf ("%.0f", 100 * ($diff/$diff_prev) - 100) . '%' ; $perc =~ s/^(\d)/\+$1/ ; }
    if ($diff >= 1000000)
    { $diff = sprintf ("%.1f", $diff / 1000000) . 'M'  ; }
    elsif ($count >= 1000)
    { $diff = sprintf ("%.0f", $diff / 1000) . 'k'  ; }
    $line .= ",$diff,$perc" ;
...
sub WriteMonthlyData
{
  ...
  foreach $f (1,2,3,4,6,11) # new editors, editors_gt_5, editors_gt_100, articles, new articles, edits
  {
    ...
    # growth in one year
    if ($totals {"$f,0"} != 0)
    { $line .= sprintf ("%.1f", 100 * ($totals {"$f,12"} / $totals {"$f,0"}) - 100). "%,"  ; }
    else
    { $line .= "n.a.," ; }

    # growth in one month
    if ($totals {"$f,11"} != 0)
    { $line .= sprintf ("%.1f", 100 * ($totals {"$f,12"} / $totals {"$f,11"}) - 100). "%,"  ; }
    else
    { $line .= "n.a.," ; }
    ...
    # print "$index $f: $key -> ${values_f_12 {$key}}\n" ;
      ...
      if ($values {"$f,0"} {$key} != 0)
      { $line .= sprintf ("%.1f", 100 * ($values {"$f,12"} {$key} / $values {"$f,0"} {$key}) - 100). "%,"  ; }
      else
      { $line .= "n.a.," ; }

      if ($values {"$f,11"} {$key} != 0)
      { $line .= sprintf ("%.1f", 100 * ($values {"$f,12"} {$key} / $values {"$f,11"} {$key}) - 100). "%,"  ; }
      else
      { $line .= "n.a.," ; }
    ...
    foreach $project (sort {$totals_project {"$f,12"} {$b} <=> $totals_project {"$f,12"} {$a}} @projects)
    {
#     next if $project eq 'commons' and ($f ==2 or $f == 3) ; # (very) active editors no longer counted for commons
      ...
      if ($totals_project {"$f,0"} {$project} != 0)
      { $line .= sprintf ("%.1f", 100 * ($totals_project {"$f,12"} {$project} / $totals_project {"$f,0"} {$project}) - 100). "%,"  ; }
      else
      { $line .= "n.a.," ; }

      if ($totals_project {"$f,11"} {$project} != 0)
      { $line .= sprintf ("%.1f", 100 * ($totals_project {"$f,12"} {$project} / $totals_project {"$f,11"} {$project}) - 100). "%,"  ; }
      else
      { $line .= "n.a.," ; }
...
#     next if $project eq 'commons' and ($f ==2 or $f == 3) ; # (very) active editors no longer counted for commons
      ...
      $value_100 = $totals_project {"$f,0"} {$project} ;
      for ($m = 0 ; $m <= 12 ; $m++)
      {
        if ($value_100 != 0)
  ...
  if ($ext_tot {0} != 0)
  { $line .= sprintf ("%.1f", 100 * ($ext_tot {12} / $ext_tot {0}) - 100). "%,"  ; }
  else
  { $line .= "n.a.," ; }

  if ($ext_tot {11} != 0)
  { $line .= sprintf ("%.1f", 100 * ($ext_tot {12} / $ext_tot {11}) - 100). "%,"  ; }
  else
  { $line .= "n.a.," ; }
  ...
  for ($e = $#extndxs ; $e >= $#extndxs - 9 ; $e--)
  {
    ...
    else
    {
      ...
      if ($ext_cnt {0}{$extndx} != 0)
      { $line .= sprintf ("%.1f", 100 * ($ext_cnt {12}{$extndx} / $ext_cnt {0}{$extndx}) - 100). "%,"  ; }
      else
      { $line .= "n.a.," ; }

      if ($ext_cnt {11}{$extndx} != 0)
      { $line .= sprintf ("%.1f", 100 * ($ext_cnt {12}{$extndx} / $ext_cnt {11}{$extndx}) - 100). "%,"  ; }
      else
      { $line .= "n.a.," ; }
    }
...
sub SetComparisonPeriods
{
  ...
  my ($month_0, $month_0_file, $month_0_minus_12, $month_0_minus_1) ;

  $year_  = $year ;
  $month_ = $month ;

  $month_0      = sprintf ("%04d/%02d",$year,  $month+1) ;
  $month_0_file = sprintf ("%04d_%02d",$year,  $month+1) ; # for filenames
  $month_0_minus_12 = sprintf ("%04d/%02d",$year-1,$month+1) ;
  ($year,$month) = $month > 1 ? ($year,$month-1) : ($year-1,12) ;
  $month_0_minus_1 = sprintf ("%04d/%02d",$year,$month+1) ;
...
#  $file_out = "$dir_out/PageViewsPerMonthPopularWikis_$month_0_file.csv" ;
...
sub MonthsSinceYearAgo
{
  my $year  = shift ;
  my $month = shift ;
  return 12 - (($year_0 - $year) * 12 + $month_0 - $month) ;
}
