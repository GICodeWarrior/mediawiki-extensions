...
// for IE excanvas.js see http://explorercanvas.blogspot.com/
  ...
  if (x_axis_mode == x_axis_editors) and (r_size_mode != r_size_mode_log) // keep large circles far right within diagram
  ...
  {  
    ...
    { 
      ...
      if (r_size_mode == r_size_mode_sqrt) // add space for largest circle 
      { articles_max *= 2 ; } // Stu: to fit wp:en fully *=2 -> *= 10
    ...
    diagram_width_log_editors5 = diagram_width / (Math.log(editors_max)/LOG10) ; 
  }   
    ...
    if (x_axis_mode == x_axis_time)
    {
      if (editors > 0) 
      {
        ...
        if (r_size_mode == r_size_mode_lin)
          return 2 + editors/editors_max * r_mult * 20 ;
        if (r_size_mode == r_size_mode_sqrt) 
          return 2 + Math.sqrt(editors/editors_max) * r_mult * 30 ;  
      }
      else    
        return 2 ;
...
//**** LOAD ****//
  ...
  {                                      
    ...
    if ((x_axis_mode == x_axis_time) && (r_size_mode != r_size_mode_log)) // keep large circles far right within diagram
    { months += 6 ; }
