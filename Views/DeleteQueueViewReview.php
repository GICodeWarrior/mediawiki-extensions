...
class DeleteQueueViewReview extends DeleteQueueView {
	...
	public function show( $params ) {
		...
		$wgOut->addWikiMsg( "deletequeue-review$queue-text",
							$article->mTitle->getPrefixedText(), $discussionPage );
		...
		$change_option = Xml::radioLabel(
									wfMsg( 'deletequeue-review-change' ),
									'wpSpeedyAction',
									'change',
									'mw-review-change'
								);
		$change_fields = array();
		$change_fields['deletequeue-review-newreason'] =
			Xml::listDropDown(
						'wpSpeedyNewReason',
			...
			foreach ( $new_queues as $option ) {
				$this_queue =
					Xml::radioLabel(
						...
						'wpSpeedyRequeue',
						$option,
						"mw-review-requeue-$option"
					);
				$disabledMsg =
					wfMsgExt( "deletequeue-requeuedisabled",
							array( 'parseinline' ),
							array( wfMsgNoTrans( "deletequeue-queue-$option" )
						) );
		...
		$dq = Xml::radioLabel(
				wfMsg( 'deletequeue-review-dequeue' ),
				'wpSpeedyAction',
				'dequeue',
				'mw-review-dequeue'
				);
		$option_selection .=
			$this->getActionOrError( 'dequeue',
					$dq,
					null,
					Xml::tags( 'li', array(), '$1' )
				);
