...
class ExtPatchOutputMobile {
	...
	public function javascriptize($s) {
		...
		if (isset($GLOBALS['headings']) && $GLOBALS['headings'] > 0) {
			$s = str_replace('<div class="visualClear">', '</div><div class="visualClear">', $s);
		}
	...
	public function DOMParse($html) {
		...
		//You can't remove DOMNodes from a DOMNodeList as you're iterating over them
		// in a foreach loop. It will seemingly leave the internal iterator on the foreach out of wack
		// and results will be quite strange. Though, making a queue of items to remove
